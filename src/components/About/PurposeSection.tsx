    ...
    <section className="w-full bg-black">
      <div className="mx-auto flex flex-col px-4 py-16 md:max-w-[1440px] md:flex-row md:justify-between md:px-24 md:py-36">
        <div className="mb-12 flex flex-col gap-8 md:mb-0 md:h-[320px] md:w-[800px]">
          ...
          <div className="flex w-full flex-col md:max-w-[480px]">
        ...
        <div className="flex flex-col gap-8 md:h-auto md:w-[440px]">
          {data?.cards.map((card, index) => {
            const styles = getCardStyles(index);
            return (
              <div
                key={index}
                className={`flex w-full flex-col gap-4 rounded-[24px] p-6 md:p-10 ${styles?.container}`}
                style={{ minHeight: "270px" }}
              >
                <div className="flex h-10 w-10 items-center justify-center rounded-full border-[1px] border-current">
                  {card.customLogo.responsiveImage ? (
                    <DatoCmsImage
                      data={card.customLogo}
                      className="h-full w-full"
                      objectFit="contain"
                    />
                  ) : (
                    <img
                      src={card.customLogo.url}
                      alt={card.customLogo.alt}
